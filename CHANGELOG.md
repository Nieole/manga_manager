...
- 补齐 LLM provider 路由与提示词文本处理的测试。其中 `NewAIProvider` 对认不出的 provider 名会**静默降级成 ollama 并丢掉 API Key**（配错一个字母，你看到的是「连不上本地模型」而不是「provider 名不认识」）——这一条按现状钉住并在用例里写清了代价，尚未改成显式报错。
