...
- **保留无数据库边界**：本阶段不做页清单持久化，不修改 SQL/schema/sqlc，只推进阅读器前端交互逻辑模块化。
...
- **保留无数据库边界**：本阶段不做页清单持久化，不修改 SQL/schema/sqlc，只推进阅读器前端交互体验收口。
...
- **保留无数据库边界**：本阶段不做页清单持久化，不修改 SQL/schema/sqlc，只新增开发/测试工具。
