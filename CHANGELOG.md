...
#### 阅读路径性能
- **新增页清单持久化**：扫描时写入书籍页面 manifest，阅读页列表和单页读取优先使用数据库定位归档 entry，减少翻页时重复解析归档目录。
...
#### 阶段 1 收口
- **新增性能基准**：补充归档页清单读取、页面读取、图片处理和大库分页查询 benchmark，便于后续发布前检查性能回归。
...
- **覆盖首批整理问题类型**：支持识别空页书籍、缺失封面、缺失元数据、缺失页清单、重复文件哈希和 KOReader 未匹配记录。
...
- **支持安全维护动作**：缺元数据可提交刮削任务，空页、缺封面和缺页清单可提交系列重扫任务，KOReader 未匹配可提交进度重关联任务；当前阶段不自动移动或删除文件。
...
- **新增 OPDS 专用页流路由**：`/opds/v1.2/books/{bookId}/pages/{pageNumber}` 使用 OPDS-PSE 0 基页码，并复用现有页面 manifest、缓存、图片处理和 ETag。
...
#### 离线阅读闭环
- **增强 Service Worker 阅读缓存策略**：阅读页、页清单和书籍信息支持网络优先、缓存兜底，非阅读 API 和 SSE 仍不进入离线缓存。
- **新增书籍级离线缓存工具**：阅读器可缓存当前书籍的页清单、书籍信息、阅读器路由和当前图像配置下的全部页图。
...
- **保留无数据库边界**：本阶段不做页清单持久化，不修改 SQL/schema/sqlc，只推进前端模块化。
...
- **保留无数据库边界**：本阶段不做页清单持久化，不修改 SQL/schema/sqlc，只推进阅读器前端模块化。
...
- **保留无数据库边界**：本阶段不做页清单持久化，不修改 SQL/schema/sqlc，只推进阅读器前端状态模块化。
...
- **保留无数据库边界**：本阶段不做页清单持久化，不修改 SQL/schema/sqlc，只推进阅读器前端运行时逻辑模块化。
