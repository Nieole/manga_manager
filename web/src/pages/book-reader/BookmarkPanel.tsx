  ...
  return (
    <div className="rounded-lg border border-gray-800 bg-gray-900/50 p-3">
      ...
      <textarea
        ...
        className="mb-2 h-16 w-full resize-none rounded-sm border border-gray-700 bg-gray-950 p-2 text-xs text-gray-300 outline-hidden focus:border-komgaPrimary"
      />
      <button
        onClick={onSaveBookmark}
        disabled={savingBookmark || loading}
        className="mb-3 flex w-full items-center justify-center gap-2 rounded-sm bg-komgaPrimary px-3 py-2 text-xs font-semibold text-white hover:bg-komgaPrimaryHover disabled:cursor-not-allowed disabled:opacity-60"
      >
      ...
      <div className="max-h-36 space-y-2 overflow-y-auto pr-1">
        {bookmarks.length === 0 ? (
          ...
          <div key={bookmark.id} className="flex items-start gap-2 rounded-sm border border-gray-800 bg-gray-950/70 p-2">
            <button
              onClick={() => onJumpToPage(bookmark.page)}
              className="shrink-0 rounded-sm bg-gray-800 px-2 py-1 text-[11px] font-semibold text-komgaPrimary hover:bg-gray-700"
            >
            ...
            <button
              ...
              className="shrink-0 rounded-sm p-1 text-gray-500 hover:bg-red-500/10 hover:text-red-300"
