 ...
 *
 * 本文件守卫「管理员专属页面不会被普通用户进到」：不守路由的话，普通用户点进设置页
 * 看到的是一屏加载失败，像系统坏了而非没权限（后端写权限已按 isRegularWritablePath
 * 收紧到管理员专属，前端仍需单独拦一道）。隐藏入口只挡「点得到」的路径，直输 URL
 * 或用旧书签仍会进去，所以这里守的是路由级守卫本身。
 */
