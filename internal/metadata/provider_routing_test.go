// 业务说明：本文件守卫 LLM провайдер 的路由表与提示词的文本处理。
//
// 这两处的共同点是**错了不报错**：
//   - NewAIProvider 的 default 分支把任何认不出来的 provider 静默降级成 ollama，
//     顺带丢掉 apiKey。配置里写错一个字母，用户看到的是「连不上本地模型」，
//     而不是「provider 名不认识」；
//   - 候选简介按字节截断会切出非法 UTF-8（中文一个字 3 字节），模型侧收到的是乱码，
//     而这一步在日志里什么都不会留下。
