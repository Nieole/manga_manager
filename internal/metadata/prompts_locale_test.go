// 本文件守卫三个提示词构造器的语言分支与结构占位：提示词错了不会报错，模型照样
// 返回东西，只是质量下降或格式不对，最终表现为「刮削结果全是空的」「AI 分组一条
// 都出不来」，排查时很难想到是语言分支写反了。
//
// 断言只钉语言判别的锚句与结构占位（候选 ID、limit 数字、无标题占位文案），
// 不断言整段文案——那样每次润色措辞都要跟着改测试。
