// 本文件集中重建所有从运行时配置派生的进程级资源（归档句柄池、图像处理并发、日志级别），
// 使「配置文件热重载」与「经 API/UI 保存配置」两条路径应用完全一致的副作用——否则会出现只有
// 其一生效的隐性 bug（例如经 UI 改 archive_pool_size / max_ai_concurrency 却不重建对应池，需重启才生效）。
