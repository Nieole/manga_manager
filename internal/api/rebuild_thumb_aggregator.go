// 「重建缩略图」任务的跨库进度聚合，以及那份**运行句柄**的保管处。
//
...
type rebuildThumbAggregator struct {
	...
	// progress 是任务体交来的运行句柄；为 nil 表示当前没有在跑重建任务，所有更新都是无操作。
...
// begin 开启一轮聚合并重置全部状态，progress 是任务体交来的运行句柄。
func (a *rebuildThumbAggregator) begin(progress *runhandle.Handle, totalLibraries int) {
	if a == nil {
		return
	}
	a.mu.Lock()
