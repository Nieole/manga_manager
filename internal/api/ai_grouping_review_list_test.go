// 业务说明：本文件守卫 AI 分组审核列表接口的查询次数与错误传播。
//
// 此前这个接口有两层 N+1：每页 30 条审核，逐条查一次候选合集（30 次），再对每个候选合集
// 逐个查一次系列名（30 × 合集数）。一次列表请求实测能发出 150+ 条单行查询。
//
// 更要命的是那两处查询的错误都被 `_` 或空返回吞掉：DB 出问题时接口照样 200，
// 候选合集静默显示为空——用户会以为 AI 一个分组都没分出来，而不是「加载失败」。
