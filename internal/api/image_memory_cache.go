// 本文件是页图/封面的内存缓存，按**总字节数**而非条数限流。
//
// 不用 golang-lru 现成库：其驱逐回调在锁外调用，字节计数与缓存内容之间没有原子性，
// 并发写入下计数必然漂移；且同键覆盖不触发驱逐回调，旧值字节会永久留在计数里，
// 几轮覆盖后计数虚高越过预算、缓存从此再也存不进任何东西。因此自己维护一个
// 「双向链表 + map」的 LRU，读写与记账在同一把锁内完成。
