...
// 业务说明：本文件守卫「外部库传输不会在目标路径上留下半截文件」。
//
// 传输的目的地通常是 USB/SD 外接盘，单本几百 MB、整个系列跑几十分钟。此前是直接往
// 最终路径写：进程被强杀或外接盘中途掉线，就会留下一个截断的 .cbz。而后续两处
// 「已存在」的判定——copyFileToExternalLibrary 开头的 os.Stat 与 external/manager.go
// 按扩展名列举归档——都只看路径不看内容，于是这本坏书在阅读器上永远打不开，
// 重传多少次都会被当成「已经有了」而跳过。
