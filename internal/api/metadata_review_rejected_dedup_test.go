// 业务说明：本文件守卫「被拒绝的提案不会被反复塞回队列」。
//
// 入队去重此前只跟 **pending** 的记录比。用户拒掉一条提案之后，下一次刮削——尤其是定时的
// 全库刮削——会把逐字相同的东西原样再塞回待审队列。用户只能反复拒绝同一条，而且每次
// 全库刮削之后收件箱都会重新涨起来，「拒绝」这个动作实际上不产生任何持久效果。
//
// 另一面同样要守：不能因此把用户堵死。误拒之后必须还有路把同一份数据重新加回队列，
// 否则「拒绝」就成了不可撤销的黑名单。
