...
// 用例据此分辨引擎选了哪条分支。
func specForTest(key string) RunSpec {
	return RunSpec{
		Key:          key,
...
// TestTaskProgressIgnoredAfterTerminal 钉住**运行句柄**的失效边界：任务已进入**终态**之后
...
// TestTaskHandleChannelsShareOneAdmissionRule 钉住**运行句柄**三条写入通道认同一条判据：
// **活动态**写得进、**终态**写不进。
//
...
func TestTaskMapsAreOwnedByTheEngine(t *testing.T) {
	...
	handle.Report(runhandle.Frame{Labels: map[string]string{"current_series": "Beta"}})
	handle.Report(runhandle.Frame{Code: "progress.scanning", Params: frameParams})
