// 守 franchise（系列关系派生的系统合集）重建后身份稳定：关系没变时合集 id 与名字不得漂移。
//
// 合集 id 对外暴露（Mihon /mihon/collections/{id}/series、OPDS /opds/collections/{id}），
// id 漂移会让客户端记的书库条目集体失效。合集名取自连通分量里 map 迭代到的「第一个」
// 成员——顺序随机，必须钉住，否则用户会看到合集名无缘无故变来变去。
