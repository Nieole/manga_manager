...
// ResolveStoragePolicy），在本包里只是一个不透明身份，除去首尾空白外原样分组与上报；传入的并发上限
// 由调用方从存储策略挑选，何时取令牌、取哪一类也由调用方决定（scanner 与 api）；诊断与暂停的
// HTTP 端点属于 api；单个任务的暂停闸门是 taskcontrol.PauseGate，与这里的后台 IO 暂停是两件事。
package storageio
