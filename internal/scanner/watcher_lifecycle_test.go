...
// TestWatchRecursiveCountsSymlinkDirs 钉住软链的处理：不注册、不算故障、但要可见。
//
// filepath.WalkDir 用的是 lstat，软链在它眼里不是目录，压根进不到注册流程；
// fsnotify 也不跟进软链目标。所以软链指向的子树确实没被监听，这一点必须让运维看得见，
// 但它又不该让整库报「监听不完整」——否则一个装饰性的软链就会制造假警报。
func TestWatchRecursiveCountsSymlinkDirs(t *testing.T) {
	if runtime.GOOS == "windows" {
	...
	if report.SymlinkDirs != 1 {
		t.Errorf("指向目录的软链应被单独计数，实际 %+v —— 这棵子树没被监听却无从知晓", report)
	}
	if report.Total != 2 { // root + real；软链不计入 Total（WalkDir 不把它当目录）
		t.Errorf("Total = %d, want 2 —— 软链不该进入注册流程的记账", report.Total)
	}
	if !report.OK() {
		t.Error("软链目录不该让整次注册被判为不完整 —— 一个装饰性的软链会误报整库失监")
	}
	fw.mu.Lock()
	_, watchedLink := fw.watched[link]
	_, watchedReal := fw.watched[real]
	fw.mu.Unlock()
	if watchedLink {
		t.Error("软链目录被注册了，但 fsnotify 不会跟进目标目录，账目虚高")
	}
	if !watchedReal {
		t.Error("真实子目录没被监听")
	}
